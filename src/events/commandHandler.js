...
module.exports = async (message) => {
  ...
  const embed = new MessageEmbed().setFooter(`Developed by Galaxy.`).setColor(message.member.displayHexColor).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, size: 2048 }));
  ...
  if (cmd) {
    ...
    if(komutLog) komutLog.wsend(new MessageEmbed().setColor("#2f3136").setTimestamp().setFooter(`Developed by Galaxy. | Kullanma Zamanı : `).setDescription(`${message.author} (\`${message.author.id}\`) tarafından ${message.channel} (\`${message.channel.id}\`) kanalında \`${prefix}${commandName}\` komutunu kullandı!\n\nKomut içeriği: \`${message.content}\`!`))
  }
};
