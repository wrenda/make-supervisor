...
module.exports = async (message) => {
  ...
  if (data) {
    ...
    message.lineReply(`${icon} Afk modundan çıktınız. **${moment.duration(Date.now() - afkData.date).format("d [gün] H [saat], m [dakika] s [saniye]")}** süredir AFK'ydınız.`).then((x) => x.delete({ timeout: 5000 }));
  }
  ...
  if (!afkData) return;
  embed.setDescription(`${icon} ${member.toString()} kullanıcısı, \`${afkData.reason}\` sebebiyle, **${moment.duration(Date.now() - afkData.date).format("d [gün] H [saat], m [dakika] s [saniye]")}** önce afk oldu!`);
