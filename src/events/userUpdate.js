...
module.exports = async (oldUser, newUser) => {
    ...
    if (oldUser.username.includes(conf.tag) && !newUser.username.includes(conf.tag)) {
      ...
      const embed = new MessageEmbed()
        ...
        .setColor("#2f3136")
        .setDescription(`
        ${icon} ${member.toString()} kullanıcısı ${conf.tag} tagını saldığı için <@&${conf.ekipRolu}> rolü alındı.
        ${tag} Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size}
         `);
      channel.wsend(embed);
      ...
      const embed = new MessageEmbed()
        ...
        .setColor("#2f3136")
        .setDescription(`
        ${icon} ${member.toString()} kullanıcısı ${conf.tag} tagını aldığı için <@&${conf.ekipRolu}> rolü verildi.
        ${tag} Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size}
    `);
      channel.wsend(embed);
