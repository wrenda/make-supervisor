...
moment.locale("tr");
const { red, green, icon } = require("../../configs/emojis.json")
...
{
message.react(red)
message.channel.send(`${icon} Mazaretli birisini belirtmek için : **.toplantı mazeretli**. Toplantı başlatmak için **.toplantı başlat**.`).then(x=>x.delete({timeout: 5000}))
return
}
...
if(!member) {
message.react(red)
message.channel.send(`${red} Kimin mazeretli olduğunu belirtmeyi unuttun dostum.`).then(x=>x.delete({timeout:5000}))
return
}
...
{
message.react(red)
message.channel.send(`${icon} Bu üye neden mazeretli ? Belirtip tekrar dener misin ?`).then(x=> x.delete({timeout:5000}))
return
}
message.react(green)
message.channel.send(`${green} ${member} kişisi ${reason} sebebiyle mazeretli olarak kaydedildi.`).then(x=>x.delete({timeout:50000}))
