...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react(red) 
    return }
    if (conf.şüphelirole.some(x => member.roles.cache.has(x))) { message.channel.send("Bu üye zaten Şüpheli!").then(x=>x.delete({timeout:5000}))
    message.react(red) 
    return }
    const reason = args.slice(2).join(" ") || "Belirtilmedi!";
    if (message.member.roles.highest.position <= member.roles.highest.position) return message.channel.send(embed.setDescription("Kendinle aynı yetkide ya da daha yetkili olan birini Şüpheli olarak tespit edemzsin!"));
    if (!member.manageable) return message.channel.send( "Bu üyeyi Şüpheliyemiyorum!");
    if (settings.jaillimit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == settings.jaillimit) 
    {
    message.react(red)
    message.channel.send("Saatlik Şüpheleme sınırına ulaştın!").then(x=>x.delete({timeout:5000})) 
    return }
    ...
    if(conf.cezapuanlog) message.guild.channels.cache.get(conf.cezapuanlog).send(`${member} üyesi \`Şüphe cezası\` alarak toplam \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanına\` ulaştı!`);
    member.roles.set(conf.şüphelirole);
    message.react(green)
    const penal = await client.penalize(message.guild.id, member.user.id, "JAIL", true, message.author.id, reason);
    message.lineReply(`${miniicon} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle Şüpelendi! `).then(x=>x.delete({timeout:50000}))
    if (settings.dmMessages) member.send(`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle Şüpelendiniz.`).catch(() => {});
    ...
    const log = embed
      ...
      .setColor("RANDOM")
      .setDescription(`
${member.toString()} Adlı Kişiye Şüpheliye Atıldı

\`●❯\` Şüpeliye Atan Kişi : ${message.author} (\`${message.author.id}\`)
\`●❯\` Ceza Sebebi: \`${reason}\`
      `)
