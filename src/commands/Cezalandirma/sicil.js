...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    data = data.map((x) => `\`#${x.id}\` **[${x.type}]** \`${moment(x.date).format("LLL")}\` tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış. (\`Aktifmi :\` ${x.active ? green : red})`).join("\n");
