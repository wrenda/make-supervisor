...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ekle", "add"].includes(args[0])) {
      ...
      message.channel.send(`${green} Başarıyla ${args[1]} tagı yasaklı taglar arasına eklendi ve ${filtered.size} kişi jaile atıldı!`).then(x=>x.delete({timeout:5000}))
      filtered.forEach(async (x) => {
        ...
        const log = new MessageEmbed()
          ...
          .setColor("#2f3136")
          .setDescription(`
${x.toString()} üyesi jaillendi! (YASAKLI TAG)
...
\`●❯\` Jail Sebebi: \`Kullanıcı adında yasaklı tag (${args[1]}) bulundurmak.\`
      ...
      if (!data.tags.includes(args[1])) return message.channel.send(`${red} Sunucunun yasaklı tagları arasında ${args[1]} tagı bulunmuyor!`).then(x=>x.delete({timeout:5000}))
      data.tags = data.tags.filter((x) => !x.includes(args[1]));
      data.save();
      const filtered = message.guild.members.cache.filter((x) => x.user.username.includes(args[1]) && conf.jailRole.some((a) => x.roles.cache.has(a)));
      message.channel.send(embed.setDescription(`${green} Başarıyla ${args[1]} tagı yasaklı taglar arasından kaldırıldı ve ${filtered.size} kişi kayıtsıza atıldı!`)).then(x=>x.delete({timeout:5000}))
      filtered.forEach(async (x) => {
        ...
        if (settings.dmMessages) x.send(`${green} **${message.guild.name}** sunucusunda, ${args[1]} tagı yasaklı taglar arasından kaldırıldı ve jailiniz açıldı.`).catch(() => {});

        const log = new MessageEmbed()
          .setAuthor(x.user.username, x.user.avatarURL({ dynamic: true, size: 2048 }))
          .setColor("#2f3136")
          .setDescription(`
${x.toString()} üyesinin jaili kaldırıldı! (YASAKLI TAG)
