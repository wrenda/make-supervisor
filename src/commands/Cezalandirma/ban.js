...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.lineReply(`${Cezaa} ${member ? member.toString() : user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle banlandı!(\`(Ceza ID: #${penal.id})\`)`).then(x=>x.delete({timeout:50000}))
    message.react(green)
    ...
    const log = embed
      .setDescription(`
\`●❯\`  Banlanan Üye: ${member ? member.toString() : ""} \`${user.id}\`
