...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react(green)
    message.lineReply(`${miniicon} \`(${ban.user.username.replace(/\`/g, "")} - ${ban.user.id})\` adlı üyenin banı ${message.author} tarafından kaldırıldı!`).then(x=>x.delete({timeout:50000}))
