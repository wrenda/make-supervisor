...
message.react(green)
message.lineReply(`${green} ${member} kişisine başarıyla <@&${conf.musician}> rolü verildi!`).then(x=>x.delete({timeout:5000}))
await member.roles.add(conf.musician)
} else {
message.react(green)
message.lineReply(`${green} ${member} kişisinden başarıyla <@&${conf.musician}> rolü alındı!`).then(x=>x.delete({timeout:5000}))
