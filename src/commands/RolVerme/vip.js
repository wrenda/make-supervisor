...
message.react(green)
message.lineReply(`${green} ${member} kişisine başarıyla VIP rolü verildi!`).then(x=>x.delete({timeout:5000}))
await member.roles.add(conf.vipRole)
} else {
message.react(green)
message.lineReply(`${green} ${member} kişisinden başarıyla VIP rolü alındı!`).then(x=>x.delete({timeout:5000}))
