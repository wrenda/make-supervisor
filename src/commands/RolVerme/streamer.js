...
message.react(green)
message.lineReply(`${green} ${member} kişisine başarıyla <@&${conf.streamer}> rolü verildi!`).then(x=>x.delete({timeout:5000}))
await member.roles.add(conf.streamer)
} else {
message.react(green)
message.lineReply(`${green} ${member} kişisinden başarıyla <@&${conf.streamer}> rolü alındı!`).then(x=>x.delete({timeout:5000}))
