...
message.react(green)
message.lineReply(`${green} ${member} kişisine başarıyla <@&${conf.tasarımcı}> rolü verildi!`).then(x=>x.delete({timeout:5000}))
await member.roles.add(conf.tasarımcı)
} else {
message.react(green)
message.lineReply(`${green} ${member} kişisinden başarıyla <@&${conf.tasarımcı}> rolü alındı!`).then(x=>x.delete({timeout:5000}))
