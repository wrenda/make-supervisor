...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt @Galaxy/ID <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
...
${uye.toString()} sunucumuza <@${message.author.id}> tarafından, Kayıtsızdan Çıkartılarak , "\`${setName}\`" ismiyle ${conf.erkekRolleri.length > 1 ? conf.erkekRolleri.slice(0, -1).map(x => `<@&${x}>`).join(", ") + " ve " + conf.erkekRolleri.map(x => `<@&${x}>`).slice(-1) : conf.erkekRolleri.map(x => `<@&${x}>`).join("")} rolleri verilerek kayıt edildi!\n
...
await button.reply.defer()
 message.lineReply(embed.setDescription(`
${uye.toString()} sunucumuza <@${message.author.id}> tarafından, Kayıtsızdan Çıkartılarak  "\`${setName}\`" ismiyle ${conf.kizRolleri.length > 1 ? conf.kizRolleri.slice(0, -1).map(x => `<@&${x}>`).join(", ") + " ve " + conf.kizRolleri.map(x => `<@&${x}>`).slice(-1) : conf.kizRolleri.map(x => `<@&${x}>`).join("")} rolleri verilerek kayıt edildi!\n
\`.isimler <@Galaxy/ID>\` Diyerek Geçmiş İsimlerini Görüntüye Bilirsin.  `)
.setFooter(`Developed by Galaxy.`)
.setAuthor(uye.displayName, uye.user.displayAvatarURL({ dynamic: true }))
.setThumbnail(uye.user.displayAvatarURL({ dynamic: true, size: 2048 })))
