...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}isim @Galaxy/ID <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
    if(message.author.id === uye.id) 
    {
    message.react(red)
    message.lineReply(`${red}\`hata:\`Kendi ismini değiştiremezsin. Booster isen \`${prefix}zengin\``).then(x => x.delete({timeout: 5000})); 
    return }
    if(!uye.manageable) 
    {
    message.react(red)
    message.lineReply(`${red}\`hata:\`Böyle birisinin ismini değiştiremiyorum.`).then(x => x.delete({timeout: 5000})); 
    return }
    if(message.member.roles.highest.position <= uye.roles.highest.position) 
    {
    message.react(red)
    message.lineReply(`${red}\`hata:\`Senden yüksekte olan birisinin ismini değiştiremezsin.`).then(x => x.delete({timeout: 5000})); 
    return }
