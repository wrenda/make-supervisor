...
const { red, green , icon , mikrafon , kulaklık } = require("../../configs/emojis.json");
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (channel) {
      ...
      ${icon} \`${channel.name}\` adlı kanaldaki üyelerin ses bilgileri:
...
**-------------**
${mikrafon}\`Mikrofonu:\` ${member.voice.mute ? `Kapalı ${red}` : `Açık ${green}`}
${kulaklık}\`Kulaklığı:\` ${member.voice.deaf ? `Kapalı ${red}` : `Açık ${green}`}
